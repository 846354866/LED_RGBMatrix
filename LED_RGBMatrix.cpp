...
// multiple LED_RGBMatrix instances.  The solution at present is to
...
static LED_RGBMatrix *activePanel = NULL;

LED_RGBMatrix::LED_RGBMatrix(
...
void LED_RGBMatrix::begin(void)
{
	...
	latport   = portOutputRegister(digitalPinToPort(_latch));
	latpin    = digitalPinToBitMask(_latch);
	oeport    = portOutputRegister(digitalPinToPort(_oe));
	oepin     = digitalPinToBitMask(_oe);
	addraport = portOutputRegister(digitalPinToPort(_a));
	addrapin  = digitalPinToBitMask(_a);
	addrbport = portOutputRegister(digitalPinToPort(_b));
	addrbpin  = digitalPinToBitMask(_b);
	addrcport = portOutputRegister(digitalPinToPort(_c));
	addrcpin  = digitalPinToBitMask(_c);
	plane     = nPlanes - 1;
	row       = nRows   - 1;
	swapflag  = false;
...
// Original LED_RGBMatrix library used 3/3/3 color.  Later version used
...
uint16_t LED_RGBMatrix::Color333(uint8_t r, uint8_t g, uint8_t b)
{
	// RRRrrGGGgggBBBbb
...
uint16_t LED_RGBMatrix::Color444(uint8_t r, uint8_t g, uint8_t b)
{
	// RRRRrGGGGggBBBBb
...
uint16_t LED_RGBMatrix::Color888(uint8_t r, uint8_t g, uint8_t b)
{
	return ((uint16_t)(r & 0xF8) << 8) | ((uint16_t)(g & 0xFC) << 3) | (b >> 3);
}

// 8/8/8 -> gamma -> 5/6/5
uint16_t LED_RGBMatrix::Color888(
...
uint16_t LED_RGBMatrix::ColorHSV(
...
void LED_RGBMatrix::customizeZH(const uint8_t *arr, uint8_t fontSize, uint8_t x, uint8_t y, uint16_t color)
{
...
void LED_RGBMatrix::drawPixel(int16_t x, int16_t y, uint16_t c)
{
...
void LED_RGBMatrix::fillScreen(uint16_t c)
{
...
uint8_t *LED_RGBMatrix::backBuffer()
{
...
void LED_RGBMatrix::swapBuffers(boolean copy)
{
...
void LED_RGBMatrix::dumpMatrix(void)
{
...
void LED_RGBMatrix::updateDisplay(void)
{
